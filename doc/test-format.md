...
- **`{{ ... }}` templating** — a value that is *exactly* `{{ expr }}` is replaced with the resolved value's real type (list/dict/scalar/bool), evaluated the same way rule expressions are (see [Rule Expressions Are Jinja](./config.md#rule-expressions-are-jinja)); a `{{ expr }}` embedded in a longer string is rendered to text instead, same as a `notice`/`notify` template. Available references: `parameter`, `variables` (the effective variables after any test override), `config` (object with `.variables` field holding the real config's variables), `account`.
