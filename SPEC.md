...
GitHub notifications (and eventually other platforms like Jira) have no
built-in filtering mechanisms. This results in notification spam drowning
out the alerts that actually matter.

## Solution

`signalsmith` provides a programmable filtering layer between notification
sources and your attention:
...
renotify timestamps) — see [doc/state.md](./doc/state.md#spool).
...
`signalsmith` provides commands for running one-time polls, continuous
daemon mode, offline testing of rules, and cache management.
